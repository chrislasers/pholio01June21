...
class DBService {
    ...
    func getAllUsers(completion: @escaping ([UserModel]) -> Void) {
        users.observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
            ...
            for (index, value) in usersArray.enumerated() {
                if value.userId == Auth.auth().currentUser?.uid {
    ...
    func getAllModels(completion: @escaping ([UserModel]) -> Void) {
        users.queryOrdered(byChild: "Usertype").queryEqual(toValue: "Model").observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
    ...
    func getAllPhotogs(completion: @escaping ([UserModel]) -> Void) {
        users.queryOrdered(byChild: "Usertype").queryEqual(toValue: "Photographer").observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
